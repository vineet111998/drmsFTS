...
//       $this->data["states"] = $this->db->query($sql)->result_array();
      // $this->load->view("listStates", $this->data);
      $sql="SELECT * FROM rpt_sif_details GROUP BY rsd_fund";
      $this->data["fundlist"] = $this->db->query($sql)->result_array();
     
     print_r($this->data["fundlist"]); die();

      // $this->data["fundlist"] = $fundlist;

      // $regionlist = $this->db->query("SELECT * FROM rpt_sif_details GROUP BY rsd_region_name")->result_array();
    ...
    public function donor_allocation(){
      ...
      $fundlist = $this->db->query("SELECT * FROM rpt_sif_details GROUP BY rsd_fund")->result_array();
      $this->data["fundlist"] = $fundlist;
    ...
    public function sif_edit(){
      $regionlist = $this->db->query("SELECT * FROM mst_rpt_sif_details GROUP BY mrsd_region_name")->result_array();
      $this->data["regionlist"] = $regionlist;
...
//======================================================================================================
        ...
        function getAnchalByRegionCode($regioncode="",$preparedrpdwn=0){
            $anchalList=array();
            $sql ="SELECT rsd_anchal_name,rsd_anchal_code FROM rpt_sif_details WHERE rsd_region_code LIKE '{$regioncode}' GROUP BY rsd_anchal_name";
          ...
          function dropdownAnchal($anchalList=array()){
            ...
            foreach ($anchalList as $anchal) {
              $str.="<option value='".$anchal['rsd_anchal_code']."'>".$anchal['rsd_anchal_name']."</option>";
            }
            return $str;
          }
          ...
          $schoollist=array();
          $sql ="SELECT * FROM rpt_sif_details WHERE rsd_anchal_code LIKE '{$anchalcode}' AND rsd_fund LIKE '{$fundingchapter}'GROUP BY rsd_school_name";
          // echo($sql);
          // die;
        ...
        function dropdownSchoolsbyAnchalDonor($schoollist=array()){
          foreach ($schoollist as $school) {
            $str.="<tr><td>".$school['rsd_state']."</td><td>".$school['rsd_region_name']."</td><td>".$school['rsd_region_code']."</td><td>".$school['rsd_anchal_name']."</td><td>".$school['rsd_anchal_code']."</td><td>".$school['rsd_school_name']."</td><td>".$school['rsd_school_code']."</td><td>".$school['rsd_fund']."</td><td><input type='checkbox' class='checkbox' name='' value=''></td></tr>";
          }
          return $str;
        }
...
//====================================================================================
          ...
          $schoollist=array();
          $sql ="SELECT * FROM rpt_sif_details WHERE rsd_anchal_code LIKE '{$anchalcode}' AND rsd_fund LIKE '' GROUP BY rsd_school_name";
          // echo($sql);
          // die;
          $schoollist=$this->db->query($sql)->result_array();
        ...
        function dropdownSchoolsbyAnchal($schoollist=array()){
          foreach ($schoollist as $school) {
            $str.="<tr><td>".$school['rsd_state']."</td><td>&emsp;&emsp;".$school['rsd_region_name']."</td><td>&emsp;&emsp;&emsp;&emsp;".$school['rsd_region_code']."</td><td>&emsp;&emsp;".$school['rsd_anchal_name']."</td><td>&emsp;".$school['rsd_anchal_code']."</td><td>".$school['rsd_school_name']."</td><td>".$school['rsd_school_code']."</td><td><input type='checkbox' class='checkbox check' name='' value='".$school['rsd_school_code']."'></td></tr>";
          }
          return $str;
        }
...
//====================================================================================
        ...
        function getAnchalByRegionCodesif($regioncode="",$preparedrpdwn=0){
            $anchalList=array();
            $sql ="SELECT mrsd_anchal_name,mrsd_anchal_code FROM mst_rpt_sif_details WHERE mrsd_region_code LIKE '{$regioncode}' GROUP BY mrsd_anchal_name";
          ...
          function dropdownAnchalsif($anchalList=array()){
            $str="<option value=''>Please select anchal</option>";
            foreach ($anchalList as $anchal) {
              $str.="<option value='".$anchal['mrsd_anchal_code']."'>".$anchal['mrsd_anchal_name']."</option>";
            }
            return $str;
          }
          ...
          $sanchList=array();
          $sql ="SELECT mrsd_sanch_name,mrsd_sanch_code FROM mst_rpt_sif_details WHERE mrsd_anchal_code LIKE '{$anchalcode}' GROUP BY mrsd_sanch_name";
        ...
        function dropdownSanchsif($sanchList=array()){
          ...
          foreach ($sanchList as $sanch) {
            $str.="<option value='".$sanch['mrsd_sanch_code']."'>".$sanch['mrsd_sanch_name']."</option>";
          }
          return $str;
        }
        ...
        function getSchoolBySanchCodesif($sanchcode="",$preparedrpdwn=0){
          $schoolList=array();
          $sql ="SELECT * FROM mst_rpt_sif_details WHERE mrsd_sanch_code LIKE '{$sanchcode}' GROUP BY mrsd_school_name";
        ...
        function dropdownSchoolsbySanchsif($schoolList=array()){
          foreach ($schoolList as $school) {
            $str.="<tr><td>".$school['mrsd_fund']."</td><td>".$school['mrsd_csr']."</td><td>".$school['mrsd_state']."</td><td>".$school['mrsd_region_name']."</td><td>".$school['mrsd_region_code']."</td><td>".$school['mrsd_anchal_name']."</td><td>".$school['mrsd_anchal_code']."</td><td>".$school['mrsd_school_name']."</td><td>".$school['mrsd_school_code']."</td><td>".$school['mrsd_fund']."</td></tr>";
          }
          return $str;
        }
        ...
        //   // echo $donorID; die();
        //   $schoolList=array();
        //   $sql ="SELECT * FROM mst_rpt_sif_details WHERE mrsd_donor_id LIKE '{$donorID}'";      
        //   $schoolList=$this->db->query($sql)->result_array();
...
//===================================================================================
        function getRegionByFundingchapter($fundingchapter="",$preparedrpdwn=0){
          $regionList=array();
          $sql ="SELECT * FROM rpt_sif_details WHERE rsd_fund LIKE '{$fundingchapter}' GROUP BY rsd_region_name";
          // echo($sql);
          // die;
        ...
        function dropdownRegion($regionList=array()){
          ...
          foreach ($regionList as $region) {
            $str.="<option value='".$region['rsd_region_code']."'>".$region['rsd_region_name']."</option>";
          }
          return $str;
        }
        ...
        function getSchoolByFundingchapter($fundingchapter="",$preparedrpdwn=0){
          $schoollist=array();
          $sql ="SELECT * FROM rpt_sif_details WHERE rsd_fund LIKE '{$fundingchapter}' GROUP BY rsd_school_name";
          // echo($sql);
          // die;
          $schoollist=$this->db->query($sql)->result_array();
        ...
        function dropdownSchoolsbyFunding($schoollist=array()){
          foreach ($schoollist as $school) {
            $str.="<tr><td>".$school['rsd_state']."</td><td>".$school['rsd_region_name']."</td><td>".$school['rsd_region_code']."</td><td>".$school['rsd_anchal_name']."</td><td>".$school['rsd_anchal_code']."</td><td>".$school['rsd_school_name']."</td><td>".$school['rsd_school_code']."</td><td>".$school['rsd_fund']."</td></tr>";
          }
          return $str;
        }
...
// ================================================================================
        function uploadFundingChapter(){
          ...
          // $schoollist=array();
          // // $sql ="SELECT * FROM rpt_sif_details WHERE rsd_anchal_code LIKE '{$anchalcode}' AND rsd_fund LIKE '' GROUP BY rsd_school_name";
          // // $sql="UPDATE rpt_sif_details SET rsd_fund = '{$fundingchapter}' WHERE "
          // $schoollist=$this->db->query($sql)->result_array();

        }

        function uploadDonorId($donorId=""){
